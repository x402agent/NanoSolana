...
enum DebugActions {
    private static let verboseDefaultsKey = "nanoclawd.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "nanoclawd.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = NanoClawdPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "NanoClawd", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = NanoClawdPaths.configURL
        guard
