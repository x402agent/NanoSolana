...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.nanoclawd.mac`) even if the shipped
    ...
    {
        if environment["NANOCLAWD_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "nanoclawd.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "nanoclawd.nixMode") { return true }
