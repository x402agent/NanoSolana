...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers open claw binary`() throws {
        ...
        let nanoclawdPath = tmp.appendingPathComponent("node_modules/.bin/nanoclawd")
        try makeExecutableForTests(at: nanoclawdPath)

        let cmd = CommandResolver.nanoclawdCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([nanoclawdPath.path, "gateway"]))
    }
    ...
    @Test func `prefers open claw binary over pnpm`() throws {
        ...
        let nanoclawdPath = binDir.appendingPathComponent("nanoclawd")
        ...
        #expect(cmd.prefix(2).elementsEqual([nanoclawdPath.path, "rpc"]))
    }
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "nanoclawd", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "nanoclawd", "health", "--json"]))
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("nanoclawd@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/nanoclawd", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "nanoclawd@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/nanoclawd'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("nanoclawd"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == nanoclawdPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
