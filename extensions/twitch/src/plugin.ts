/**
 * Twitch channel plugin for NanoClawd.
 *
...
import { buildChannelConfigSchema } from "nanoclawd/plugin-sdk/twitch";
 ...
 * for NanoClawd. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: NanoClawdConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: NanoClawdConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: NanoClawdConfig): boolean => {
