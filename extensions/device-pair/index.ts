...
} from "nanoclawd/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: NanoClawdPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.NANOCLAWD_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: NanoClawdPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([
      process.env.NANOCLAWD_GATEWAY_TOKEN,
      process.env.TAMAGOBOT_GATEWAY_TOKEN,
      cfg.gateway?.auth?.token,
    ]) ?? undefined;
  const password =
    pickFirstDefined([
      process.env.NANOCLAWD_GATEWAY_PASSWORD,
...
async function resolveGatewayUrl(api: NanoClawdPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the NanoClawd iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the NanoClawd iOS app:",
            "",
            "```",
            qrAscii,
            "```",
