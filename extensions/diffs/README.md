...
Read-only diff viewer plugin for **NanoClawd** agents.
...
Set plugin-wide defaults in `~/.nanoclawd/nanoclawd.json`:
...
Before:
NanoClawd supports plugins.

After:
NanoClawd supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/nanoclawd-diffs`).
