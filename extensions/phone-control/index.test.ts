...
import type {
  NanoClawdPluginApi,
  NanoClawdPluginCommandDefinition,
  PluginCommandContext,
} from "nanoclawd/plugin-sdk/phone-control";
...
function createApi(params: {
  ...
  registerCommand: (command: NanoClawdPluginCommandDefinition) => void;
}): NanoClawdPluginApi {
  return {
    id: "phone-control",
    name: "phone-control",
...
describe("phone-control plugin", () => {
  it("arms sms.send as part of the writes group", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nanoclawd-phone-control-test-"));
    try {
      ...
      let command: NanoClawdPluginCommandDefinition | undefined;
