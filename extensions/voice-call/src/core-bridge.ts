...
function resolveNanoClawdRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.NANOCLAWD_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set NANOCLAWD_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveNanoClawdRoot(), "dist", "extensionAPI.js");
