...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__nanoclawd_channel_id";
...
type InteractionCallbackConfig = Pick<NanoClawdConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "nanoclawd-mattermost-interactions").update(botToken).digest("hex");
}
