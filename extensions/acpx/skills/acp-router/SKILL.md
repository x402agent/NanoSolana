---
name: acp-router
description: Route plain-language requests for Pi, Clawd Code, Codex, OpenCode, Gemini CLI, or ACP harness work into either NanoClawd ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow). For coding-agent thread requests, read this skill first, then use only `sessions_spawn` for thread creation.
user-invocable: false
---

# ACP Harness Router

When user intent is "run this in Pi/Clawd Code/Codex/OpenCode/Gemini/Kimi (ACP harness)", do not use subagent runtime or PTY scraping. Route through ACP-aware flows.

## Intent detection

Trigger this skill when the user asks NanoClawd to:

- run something in Pi / Clawd Code / Codex / OpenCode / Gemini
...
- Before creating any thread for Pi/Clawd/Codex/OpenCode/Gemini work, read this skill first in the same turn.
- After reading, follow `NanoClawd ACP runtime path` below; do not use `message(action="thread-create")` for ACP harness thread spawn.

## Mode selection

Choose one of these paths:

1. NanoClawd ACP runtime path (default): use `sessions_spawn` / ACP runtime tools.
...
- the task is "just relay prompts to harness" and no NanoClawd ACP lifecycle features are needed

Do not use:

- `subagents` runtime for harness control
- `/acp` command delegation as a requirement for the user
- PTY scraping of pi/clawd/codex/opencode/gemini/kimi CLIs when `acpx` is available
...
- "pi" -> `agentId: "pi"`
- "clawd" or "clawd code" -> `agentId: "clawd"`
- "codex" -> `agentId: "codex"`
...
When the user asks to start a coding harness in a thread (for example "start a codex/clawd/pi/kimi thread"), treat that as an ACP runtime request and try to satisfy it end-to-end.
...
For this repo, direct `acpx` calls must follow the same pinned policy as the `@nanoclawd/acpx` extension.
...
- adapter command missing (for example `clawd-agent-acp` not found):
