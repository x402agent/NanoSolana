/**
 * Solana Clawd Go Agent Birth Certificate — Devnet Gasless NFT via Metaplex
 *
...
const NFT_COLLECTION_NAME = "Solana Clawd Go TamaGObot Birth Certificates";
...
function buildOffchainMetadata(meta: BirthCertificateMetadata): object {
  return {
    ...
    symbol: NFT_SYMBOL,
    description: `Birth certificate for Solana Clawd Go TamaGObot agent ${meta.agentId}. Born ${new Date(meta.bornAt).toISOString()}.`,
    image: "https://solana-clawd-go.com/birth-cert.png",
    external_url: "https://solana-clawd-go.com",
    attributes: [
...
export async function mintBirthCertificate(
  ...
  const metadataUri = `https://solana-clawd-go.com/api/birth-cert/${metadata.agentId}`;
