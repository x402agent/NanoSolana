# Clawd Agent SDK Deep Dive

Findings from reverse-engineering `@anthropic-ai/clawd-agent-sdk` v0.2.29–0.2.34 to understand how `query()` works, why agent teams subagents were being killed, and how to fix it. Supplemented with official SDK reference docs.
...
Agent Runner (our code)
  └── query() → SDK (sdk.mjs)
        └── spawns CLI subprocess (cli.js)
              └── Clawd API calls, tool execution
...
Each invocation = one API call to Clawd (one "turn").
...
| `additionalDirectories` | `string[]` | `[]` | Additional directories Clawd can access |
...
| `model` | `string` | Default from CLI | Clawd model to use |
| `outputFormat` | `{ type: 'json_schema', schema: JSONSchema }` | `undefined` | Structured output format |
| `pathToClawdCodeExecutable` | `string` | Uses built-in | Path to Clawd Code executable |
...
| `settingSources` | `SettingSource[]` | `[]` (none) | Which filesystem settings to load. Must include `'project'` to load CLAWD.md |
| `stderr` | `(data: string) => void` | `undefined` | Callback for stderr output |
| `systemPrompt` | `string \| { type: 'preset'; preset: 'clawd_code'; append?: string }` | `undefined` | System prompt. Use preset to get Clawd Code's prompt, with optional `append` |
| `tools` | `string[] \| { type: 'preset'; preset: 'clawd_code' }` | `undefined` | Tool configuration |
...
// 'user'    → ~/.clawd/settings.json
// 'project' → .clawd/settings.json (version controlled)
// 'local'   → .clawd/settings.local.json (gitignored)
```
...
| `assistant` | — | Clawd's response (text + tool calls) |
...
Clawd responded with text only — it decided it has completed the task. The API's `stop_reason` will be `"end_turn"`. The SDK does NOT make this decision — it's entirely driven by Clawd's model output.
...
2. assistant/user       → Clawd reasoning, tool calls, tool results
