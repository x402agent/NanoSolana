...
export class EventBus {
  ...
  /** Publish an event */
  emit(type: SwarmEventType, source: string, data: unknown): SwarmEvent {
    ...
    if (handlers) {
      for (const handler of handlers) {
        try {
          ...
          log.error(`Handler error for ${type}: ${String(err)}`);
        }
      }
    }
    ...
    for (const handler of this.wildcardHandlers) {
      try {
        ...
        log.error(`Wildcard handler error: ${String(err)}`);
      }
    }
  ...
  getEventsBySource(source: string, limit = 50): SwarmEvent[] {
