...
export class BotManager {
  ...
  async start(botId: BotId): Promise<void> {
    ...
    if (missing.length > 0) {
      throw new Error(
        `${botId}: Missing required env vars: ${missing.join(", ")}`,
      );
    }
    ...
    try {
      ...
      if (!cmd) throw new Error(`Invalid start command for ${botId}`);
      ...
      child.on("exit", (code, signal) => {
        ...
        this.emitBotEvent(botId, "bot:stopped", {
          botId,
          exitCode: code,
          signal,
        });
  ...
  getHealth(botId: BotId): BotHealth {
    ...
    return {
      ...
      uptime: instance.startedAt
        ? (Date.now() - instance.startedAt) / 1000
        : 0,
      ...
      if (def.healthEndpoint && def.port) {
        try {
          const resp = await fetch(
            `http://localhost:${def.port}${def.healthEndpoint}`,
          );
          if (!resp.ok) {
            this.appendLog(
              botId,
  ...
  /** Read the current .env file for a bot */
  ...
  setEnvConfig(botId: BotId, updates: Record<string, string>): void {
    ...
    log.info(
      `Updated env config for ${botId} (${Object.keys(updates).length} vars)`,
    );
  }
