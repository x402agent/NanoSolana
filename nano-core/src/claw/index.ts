...
async function startMessageLoop(): Promise<void> {
  ...
  logger.info(`Clawd running (trigger: @${ASSISTANT_NAME})`);
...
function ensureDockerRunning(): void {
  ...
  // Kill and clean up orphaned Clawd containers from previous runs
  try {
    const output = execSync('docker ps --filter "name=clawd-" --format "{{.Names}}"', {
...
if (isDirectRun) {
  main().catch((err) => {
    logger.error({ err }, 'Failed to start Clawd');
    process.exit(1);
  });
}
