...
async function startMessageLoop(): Promise<void> {
  ...
  logger.info(`SCG running (trigger: @${ASSISTANT_NAME})`);
...
function ensureDockerRunning(): void {
  ...
  // Kill and clean up orphaned SCG containers from previous runs
  try {
    const output = execSync('docker ps --filter "name=SCG-" --format "{{.Names}}"', {
...
if (isDirectRun) {
  main().catch((err) => {
    logger.error({ err }, 'Failed to start SCG');
    process.exit(1);
  });
}
