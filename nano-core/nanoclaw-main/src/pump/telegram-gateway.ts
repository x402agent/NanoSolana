...
export class TelegramGateway {
  ...
  private async cmdSpawn(cmd: GatewayCommand): Promise<void> {
    ...
    if (!role) {
      ...
      msg += 'Usage: <code>/spawn &lt;role&gt; [args...]</code>\n\n';
    ...
    try {
      const request = buildSpawnRequest(role, cmd.args.slice(1));
  ...
  private async cmdAgents(cmd: GatewayCommand): Promise<void> {
    ...
    for (const agent of agents) {
      ...
      msg += `${icon} <b>${agent.identity.name}</b>\n`;
      msg += `   ID: <code>${agent.identity.id}</code>\n`;
      msg += `   Role: ${agent.identity.role} | ⏱ ${uptime}\n`;
