...
describe('registry resolution', () => {
  it('prefers explicit registry over discovery/cache', async () => {
    ...
    discoverRegistryFromSite.mockResolvedValue({ apiBase: 'https://hub.nanoclawd.com' })
  ...
  it('ignores legacy registry and updates cache from discovery', async () => {
    ...
    expect(registry).toBe('https://hub.nanoclawd.com')
    expect(writeGlobalConfig).toHaveBeenCalledWith({
      registry: 'https://hub.nanoclawd.com',
      token: 'tkn',
    })
  })
