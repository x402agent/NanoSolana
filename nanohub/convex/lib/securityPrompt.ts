...
export const SECURITY_EVALUATOR_SYSTEM_PROMPT = `You are a security evaluator for NanoClawd AI skills. Users install skills to extend what their AI agent can do. Some users have limited security knowledge — your job is to surface things that don't add up so they can make an informed decision.
...
export function assembleEvalUserMessage(ctx: SkillEvalContext): string {
  ...
  const nanoclawdFallback =
    meta.nanoclawd && typeof meta.nanoclawd === 'object' && !Array.isArray(meta.nanoclawd)
      ? (meta.nanoclawd as Record<string, unknown>)
      : {}
  const clawdis = Object.keys(rawClawdis).length > 0 ? rawClawdis : nanoclawdFallback
  const requires = (clawdis.requires ?? nanoclawdFallback.requires ?? {}) as Record<string, unknown>
