...
1. `npx scg go` (or `init` + `birth` + `run`) starts your runtime.
2. That runtime includes wallet, pet, ScgVault memory, trading engine, and
   gateway.
...
- Contains wallet management, ScgVault, trading engine, gateway, NanoBot, and
  on-chain identity
- Ships `scg pay` for on-chain invoice creation and verification via
...
- extension metadata merged from `scg-plugin.json`,
  `openclaw.plugin.json`, and `package.json#scg`
...
- canonical host: `https://hub.scg.com`
...
- port: `18790`
- auth: HMAC-SHA256
- startup path: `scg run`, `scg go`, or `npm run gateway` inside
  `nano-core`
