...
The current published CLI does not expose a nested `scg wallet ...` tree.
...
- `scg pay invoice` creates payment invoices tied to the agent wallet
- `scg pay verify` verifies on-chain invoice payments
- `scg pay status` shows payment configuration
...
- By default the key stays in `~/.scg/vault.enc`.
