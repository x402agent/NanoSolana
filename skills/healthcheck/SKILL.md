---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for NanoClawd deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, NanoClawd cron scheduling for periodic checks, or version status checks on a machine running NanoClawd (laptop, workstation, Pi, VPS).
---

# NanoClawd Host Hardening

## Overview

Assess and harden the host running NanoClawd, then align it to a user-defined risk tolerance without breaking access. Use NanoClawd security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim NanoClawd changes the host firewall, SSH, or OS updates; it does not.
...
5. NanoClawd gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if NanoClawd can access sensitive data.
...
### 2) Run NanoClawd security audits (read-only)

As part of the default read-only checks, run `nanoclawd security audit --deep`. Only offer alternatives if the user requests them:

1. `nanoclawd security audit` (faster, non-probing)
2. `nanoclawd security audit --json` (structured output)

Offer to apply NanoClawd safe defaults (numbered):

1. `nanoclawd security audit --fix`

Be explicit that `--fix` only tightens NanoClawd defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check NanoClawd version/update status (read-only)

As part of the default read-only checks, run `nanoclawd update status`.
...
- Credential hygiene notes (location of NanoClawd creds, prefer disk encryption)
...
- NanoClawd security audit (re-run)
...
After NanoClawd install or first hardening pass, run at least one baseline audit and version check:

- `nanoclawd security audit`
- `nanoclawd security audit --deep`
- `nanoclawd update status`

Ongoing monitoring is recommended. Use the NanoClawd cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `nanoclawd cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `nanoclawd update status`
...
Before creating, `nanoclawd cron list` and match on exact `name`. If found, `nanoclawd cron edit <id> ...`.
If not found, `nanoclawd cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `nanoclawd update status` (preferred for source checkouts and channels)
2. `npm view nanoclawd version` (published npm version)

## NanoClawd command accuracy

Use only supported commands and flags:

- `nanoclawd security audit [--deep] [--fix] [--json]`
- `nanoclawd status` / `nanoclawd status --deep`
- `nanoclawd health --json`
- `nanoclawd update status`
- `nanoclawd cron add|list|runs|run`

Do not invent CLI flags or imply NanoClawd enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by NanoClawd compaction:
